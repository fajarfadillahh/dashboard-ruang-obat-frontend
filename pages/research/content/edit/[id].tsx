...
export default function EditResearch({
  ...
  const { data, error, isLoading } = useSWR<SuccessResponse<Research>>({
    url: `/admin/research/${encodeURIComponent(params?.id as string)}`,
    method: "GET",
    token,
  });
  ...
  async function handleEditResearch() {
    ...
    try {
      ...
      formData.append("research_id", data?.data.research_id as string);
      ...
      toast.success("Kelas riset berhasil diubah");
      router.back();
    } catch (error: any) {
      setLoading(false);
      console.error(error);
  ...
  if (error) {
    return (
      <Layout title="Edit Kelas Riset Farmasi">
        <Container>
          <ErrorPage
            {...{
  ...
  return (
    <Layout title="Edit Kelas Riset Farmasi" className="scrollbar-hide">
      <Container>
