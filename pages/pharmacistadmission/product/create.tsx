...
import { PharmacistAdmissionUniversityResponse } from "@/types/pharmacistadmission.type";
...
export default function CreatePharmacistAdmissionProduct({
  data: university,
  token,
  ...
  useEffect(() => {
    ...
    const isFormValid =
      thumbnailType === "image"
        ? [...requiredFields, fileImg, selectUniv].every(Boolean)
        : [...requiredFields, videoUrl, selectUniv].every(Boolean);

    setButtonDisabled(!isFormValid);
  }, [input, fileImg, videoUrl, thumbnailType, selectUniv]);
  ...
  async function handleCreateProduct() {
    ...
    try {
      ...
      formData.append("university_id", selectUniv);
      ...
      if (thumbnailType === "image") {
        ...
        formData.append("thumbnail_pa", fileConvert);
      } else {
        formData.append("video_url", videoUrl);
      }

      await fetcher({
        url: "/admin/pa/products",
        method: "POST",
        data: formData,
        file: true,
        token,
            ...
            <div className="grid grid-cols-[300px_1fr] items-start gap-8 pt-8">
              ...
              <div className="grid gap-4">
                ...
                <Select
                  ...
                  selectedKeys={new Set([selectUniv])}
                  onSelectionChange={(keys: any) => {
                    const selectedValue = Array.from(keys).join(",");
                    setSelectUniv(selectedValue);
                  }}
                  listboxProps={{
                    itemClasses: {
                ...
                >
                  {(university?.pharmacist_admission || []).map((univ) => (
                    <SelectItem key={univ.university_id}>
                      {univ.name}
                    </SelectItem>
                  ))}
                </Select>
