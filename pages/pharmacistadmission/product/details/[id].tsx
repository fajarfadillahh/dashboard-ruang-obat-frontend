...
export default function DetailsPharmacistAdmissionProductPage({
  params,
  token,
}: InferGetServerSidePropsType<typeof getServerSideProps>) {
  const { data, error, isLoading } = useSWR<
    SuccessResponse<PharmacistAdmissionProduct>
  >({
    url: `/admin/pa/products/${encodeURIComponent(params.id as string)}`,
    method: "GET",
    token,
  });
      ...
      <Container>
        <section className="grid gap-8">
          ...
          <div className="grid gap-8 pb-12">
            <TitleText
              title="Detail Kelas 📦"
              text="Anda bisa melihat data produk kelas lebih detail disini"
            />
