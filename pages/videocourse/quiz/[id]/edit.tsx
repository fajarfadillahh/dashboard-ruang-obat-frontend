...
import { QuizDetailResponse } from "@/types/quiz/quiz.type";
...
export default function EditQuizPage({
  ...
  const { data, error, mutate } = useSWR<SuccessResponse<QuizDetailResponse>>({
    url: getUrl(`/quizzes/${encodeURIComponent(params?.id as string)}/detail`, {
      page,
    }),
    method: "GET",
    token,
  ...
  async function handleEditQuizData() {
    setLoading(true);
    ...
    try {
      ...
      mutate();
      toast.success("Data kuis berhasil diedit!");
    } catch (error: any) {
      setLoading(false);
      console.error(error);
  ...
  async function handleDeleteQuestion(ass_id: string, assq_id: string) {
    try {
      await fetcher({
        url: `/quizzes/${ass_id}/questions/${assq_id}`,
        method: "DELETE",
        token,
      });
  ...
  return (
    <Layout title="Edit Kuis" className="scrollbar-hide">
      <Container className="gap-8">
        ...
        <div className="divide-y-2 divide-dashed divide-gray/20">
          <TitleText
            title="Edit Kuis ✏️"
            text="Saatnya edit kuis sekarang"
            className="pb-8"
          />
                ...
                <ModalConfirm
                  ...
                  body={
                    <p className="leading-[170%] text-gray">
                      Apakah anda ingin menyimpan data kuis ini ke dalam
                      database?
                    </p>
                  }
                  footer={(onClose: any) => (
              ...
              <div className="grid pt-12">
                ...
                <div className="grid gap-2 overflow-y-scroll scrollbar-hide">
                  {data?.data.questions.length ? (
                    ...
                    <EmptyData text="Belum ada soal dikuis ini!" />
                  )}
                </div>
