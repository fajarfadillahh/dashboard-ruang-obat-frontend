...
export default function CreateEventTestApotekerclassPage({
  token,
  provicesData,
  ...
  async function handleAddEventTestApotekerclass() {
    setLoading(true);
    ...
    try {
      ...
      toast.success("Event ujian berhasil dibuat!");
      router.push("/apotekerclass/events");
    } catch (error: any) {
      console.error(error);
        ...
        <div className="grid grid-cols-[350px_1fr] items-start gap-16 pt-8">
          ...
          <div className="grid gap-6">
            ...
            <Button
              ...
              onClick={handleAddEventTestApotekerclass}
              className="mt-12 justify-self-end font-semibold"
            >
              {loading ? "Tunggu Sebentar..." : "Buat Event Ujian"}
            </Button>
          </div>
        </div>
      </Container>
...
export const getServerSideProps = withToken(async () => {
  ...
  let provicesData: { name: string }[] = await response.json();
